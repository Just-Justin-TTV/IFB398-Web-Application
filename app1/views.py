...
def generate_pdf_report(request, context, project):
    ...
    print("=== PDF DOWNLOAD FUNCTION CALLED ===")  # Debug line
    print(f"Project: {project.project_name}")  # Debug line
    
    try:
        # For now, return a text file since reportlab isn't installed
        ...
        BUILDING METRICS:
        - GIFA: {context['metrics'].gifa_m2 if context['metrics'] else 'N/A'} m²
        - Roof Area: {context['metrics'].roof_area_m2 if context['metrics'] else 'N/A'} m²
        - Building Type: {context['metrics'].building_type if context['metrics'] else 'N/A'}
        ...
        print(f"PDF Error: {e}")  # Debug line
        # Ultimate fallback
        response = HttpResponse(f"PDF generation not available. Error: {str(e)}", content_type='text/plain')
        response['Content-Disposition'] = f'attachment; filename="{project.project_name}_error.txt"'
        return response
...
def generate_word_report(request, context, project):
    ...
    try:
        ...
        doc.add_paragraph(f'Report ID: {project.id}-REPORT')
        
        # Add building metrics
        if context['metrics']:
            doc.add_heading('Building Metrics', level=1)
            doc.add_paragraph(f'GIFA: {context["metrics"].gifa_m2 or "N/A"} m²')
            doc.add_paragraph(f'Roof Area: {context["metrics"].roof_area_m2 or "N/A"} m²')
            doc.add_paragraph(f'Building Type: {context["metrics"].building_type or "N/A"}')
        
        # Add financial overview
        doc.add_heading('Financial Overview', level=1)
        doc.add_paragraph(f'Total Budget: ${context["global_budget"]:,}')
        doc.add_paragraph(f'Budget Utilization: {context["budget_utilization"]}%')
        doc.add_paragraph(f'Interventions Analyzed: {context["interventions_count"]}')
        
        # Add interventions
        doc.add_heading('Recommended Interventions', level=1)
        for class_name, interventions in context['interventions_by_class'].items():
            doc.add_heading(class_name, level=2)
            for intervention in interventions:
                p = doc.add_paragraph()
                p.add_run(f'{intervention["name"]} - ').bold = True
                p.add_run(f'Cost: {intervention["cost_range"]}, Impact: {intervention["impact_rating"]}/10')
        ...
        return redirect('generate_report', project_id=project.id)
